...
1. 下载发行版：在 Releases 页面获取 `Live2DPet.exe`，双击即可运行。  
2. 从源码启动：在项目目录执行 `npm install`，然后运行 `node launch.js`。  
3. 首次启动完成 API 配置：`base_url`、`api_key`、`model`。  
4. 选择一个 Live2D 模型或图片模型后，点击「启动宠物」。  
5. 其他语言说明请使用顶部语言导航，或查看 [Global Quick Start](QUICK_START_GLOBAL.md)。
...
### 1. 记忆系统增强
- 记忆持久化到本地文件，应用重启后仍可保留
- 支持配置记忆容量与短期/长期检索数量
- 支持自动保存，降低手动维护成本
- 支持按相关性检索上下文，减少对话信息丢失
